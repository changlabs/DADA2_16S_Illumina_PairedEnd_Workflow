---
title: "Step 9: Phyloseq Object Construction"
output:
  github_document:
    toc: true
    toc_depth: 3
...
# Execute object construction once for HTML. The GitHub document is a
# code/reference view; disabling evaluation there avoids rewriting outputs and
# trying to screenshot interactive widgets in a non-HTML format.
...
This notebook is **Step 9**, the final step, of the 16S rRNA sequencing data processing pipeline. It constructs one or more [phyloseq](https://joey711.github.io/phyloseq/) objects from the ASV count table(s) and taxonomy assignments generated earlier in the pipeline. The phyloseq object is a powerful data structure that integrates sample data, taxonomy, and optionally phylogenetic trees for comprehensive microbiome analysis.

Because this notebook runs last, it automatically detects and picks up the output of every optional upstream step that was run: for each taxonomy database detected on disk ([SILVA](https://www.arb-silva.de/) and/or [GTDB](https://gtdb.ecogenomic.org/) -- whichever one or both were produced by Step 5), it builds **up to three phyloseq objects** — one from **raw counts** (Step 5, always available), one from **copy-number-corrected counts** (Step 7, if that optional notebook was run), and one from **microbial-load-corrected counts** (Step 8, if that optional notebook was run). Any combination whose input file is not found is skipped automatically, with a clear message explaining why. Every database's results — phyloseq objects, barplots, and summary workbook — are kept in their own subfolder, so SILVA- and GTDB-based outputs never mix.

## Prerequisites

Before running this notebook, ensure that:

1.  **[Step 5 (DADA2 Pipeline)](5_dada2_pipeline.md)** has been completed successfully.
2.  The ASV count table ([asv_count_table.csv](../../results/5_dada2_pipeline/asv_count_table.csv)) is present in [results/5_dada2_pipeline/](../../results/5_dada2_pipeline/).
3.  At least one taxonomy table is present ([silva_taxonomy_table.csv](../../results/5_dada2_pipeline/silva_taxonomy_table.csv) and/or [gtdb_taxonomy_table.csv](../../results/5_dada2_pipeline/gtdb_taxonomy_table.csv)).
4.  **Optional**: [Step 7 (Copy Number Correction)](7_copy_number_correction.md) has been run, to additionally build a copy-number-corrected phyloseq object per taxonomy database.
5.  **Optional**: [Step 8 (Microbial Load Correction)](8_microbial_load_correction.md) has been run, to additionally build a microbial-load-corrected phyloseq object per taxonomy database.
6.  **Optional**: Sample metadata file in TSV (or CSV) format -- see [data/README.md](../../data/README.md) for the required layout. The ready-to-edit [data/metadata.tsv](../../data/metadata.tsv) contains ten generic placeholder samples (`S01`–`S10`); replace its rows and values with your own before drawing conclusions from metadata-based groupings.
7.  **Optional**: Phylogenetic trees from [Step 6](6_phylogenetic_tree.md).

## What This Notebook Does

The workflow accomplishes the following tasks:

1.  **Import Data**: Loads whichever abundance table(s) are available (raw, copy-number-corrected, microbial-load-corrected), taxonomy assignments, and optional metadata/phylogenetic tree.
2.  **Construct Phyloseq Objects**: Creates one integrated data structure per (taxonomy database x available abundance source) combination.
3.  **Data Exploration**: Summarizes the composition and taxonomy of every phyloseq object built.
4.  **Taxonomic Aggregation**: Aggregates data at the Genus level for every phyloseq object.
5.  **Relative Abundance**: Calculates relative abundance transformations.
6.  **Visualization**: Generates an interactive top-taxa Genus-level barplot for each phyloseq object that contains at least one classified genus, using a shared genus-to-color mapping.
7.  **Export**: Saves every phyloseq object for downstream analysis.

## Expected Input

-   **Always required**, from [results/5_dada2_pipeline/](../../results/5_dada2_pipeline/) (Step 5):
    -   [asv_count_table.csv](../../results/5_dada2_pipeline/asv_count_table.csv) — raw ASV abundance matrix.
    -   [silva_taxonomy_table.csv](../../results/5_dada2_pipeline/silva_taxonomy_table.csv) and/or [gtdb_taxonomy_table.csv](../../results/5_dada2_pipeline/gtdb_taxonomy_table.csv) — taxonomy assignments.
-   **Optional**, automatically included if present:
    -   [results/7_copy_number_correction/copy_number_corrected_asv_count_table.csv](../../results/7_copy_number_correction/copy_number_corrected_asv_count_table.csv) (Step 7).
    -   [results/8_microbial_load_correction/microbial_load_corrected_abundance_table.csv](../../results/8_microbial_load_correction/microbial_load_corrected_abundance_table.csv) (Step 8).
    -   [results/6_phylogenetic_tree/phylogenetic_tree.nwk](../../results/6_phylogenetic_tree/phylogenetic_tree.nwk) (Step 6).
    -   Sample metadata TSV (or CSV) file -- see [data/README.md](../../data/README.md).

## Expected Output

Every output below is split into one self-contained subfolder per taxonomy database under [results/9_phyloseq_object/](../../results/9_phyloseq_object/) -- e.g. [results/9_phyloseq_object/SILVA/](../../results/9_phyloseq_object/SILVA/) and/or [results/9_phyloseq_object/GTDB/](../../results/9_phyloseq_object/GTDB/) -- rather than a single flat folder mixing both databases together, since a phyloseq object (and everything derived from one) is always specific to exactly one taxonomy database. Within each database's own subfolder, the `.RData` objects and the interactive barplots are further split into their own [phyloseq_objects/](../../results/9_phyloseq_object/SILVA/phyloseq_objects/) and [barplots/](../../results/9_phyloseq_object/SILVA/barplots/) subfolders.

-   Up to three phyloseq objects per taxonomy database, saved as `.RData` files (one per available abundance source), e.g. [SILVA/phyloseq_objects/phyloseq_object_silva_raw_counts.RData](../../results/9_phyloseq_object/SILVA/phyloseq_objects/phyloseq_object_silva_raw_counts.RData).
-   One interactive HTML barplot per phyloseq object containing classified genera -- the top `n_top_taxa` genera by relative abundance, excluding unclassified/NA genera. Objects with no classified genus are retained, but their top-genus table and plot are skipped with a warning.
-   A summary statistics Excel workbook per taxonomy database, e.g. [SILVA/phyloseq_summary_silva.xlsx](../../results/9_phyloseq_object/SILVA/phyloseq_summary_silva.xlsx), containing `Tax_Summary`, per-source `Top_Genera_*`/`Sample_Stats_*`, `Run_Provenance`, and a trailing `Column_Dictionary` sheet.
-   Genus-level aggregated objects are intermediate in-memory objects used to generate the summary tables and barplots; they are not separate disk outputs.
...
::: {.alert .alert-info}
**Before running this notebook**: install the required R packages by running [setup/install_R_dependencies.R](../../setup/install_R_dependencies.R). Complete [Step 5](5_dada2_pipeline.md) at minimum; [Step 6](6_phylogenetic_tree.md), [Step 7](7_copy_number_correction.md), and [Step 8](8_microbial_load_correction.md) are optional and are picked up automatically if present.
:::
...
cat("Path Configuration:\n",
    "- DADA2 results folder:", dada2_results_folder, "\n",
    "- Tree results folder:", tree_results_folder, "\n",
    "- Copy-number-correction results folder:", copy_number_results_folder, "\n",
    "- Microbial-load-correction results folder:", microbial_load_results_folder, "\n",
    "- Final output folder:", final_output_folder, "\n",
    "- Per-database subfolders and Excel workbooks: created automatically below,\n",
    "  once Load Taxonomy Table(s) has determined which database(s) are available\n")
```

## Define Data Selection Parameters {#define-data-params}

Configure the optional components to include. Which **taxonomy database(s)** (SILVA / GTDB) and **abundance sources** (raw / copy-number-corrected / microbial-load-corrected) get included is not configured here — both are detected automatically, in [Load Taxonomy Table(s)](#load-taxonomy) and [Load Abundance Tables](#load-abundance) below respectively, based on which output files are actually present on disk.

```{r define-data-params}
...
This notebook can automatically build phyloseq objects from up to three abundance tables. **Raw counts** (Step 5) are always required; **copy-number-corrected** (Step 7) and **microbial-load-corrected** (Step 8) counts are picked up automatically if their output files exist, and skipped -- with a message, not an error -- otherwise.

```{r define-abundance-sources}
...
```{r echo=FALSE}
cat("\nAbundance sources available for this run:", paste(names(abundance_tables), collapse = ", "), "\n")

# Optional corrected tables must describe the same biological samples as the
# required raw table; ASV sets may differ because Step 7 can exclude ASVs.
raw_sample_ids <- rownames(abundance_tables[["raw_counts"]])
for (source_key in setdiff(names(abundance_tables), "raw_counts")) {
    source_sample_ids <- rownames(abundance_tables[[source_key]])
    if (!setequal(raw_sample_ids, source_sample_ids)) {
        stop("Abundance source '", source_key, "' does not contain the same sample set as raw_counts.")
    }
    unexpected_asvs <- setdiff(colnames(abundance_tables[[source_key]]),
                               colnames(abundance_tables[["raw_counts"]]))
    if (length(unexpected_asvs)) {
        stop("Abundance source '", source_key,
             "' contains ASVs absent from the current raw Step 5 table: ",
             paste(head(unexpected_asvs, 20), collapse = ", "))
    }
}
```
...
This chunk automatically detects which taxonomy database(s) Step 5 produced -- SILVA, GTDB, or both -- and loads whichever are actually present, exactly mirroring the auto-detection already used for the abundance sources above ([Load Abundance Tables](#load-abundance)). A database whose file is missing is skipped with a message, not an error; the chunk only stops if *neither* database's file can be found, since a phyloseq object cannot be built without at least one taxonomy assignment.

```{r load-taxonomy-tables}
...
for (db_name in names(taxonomy_files)) {
    # row.names = 1 uses the first column (ASV ID) as row names, matching
    # the ASV-ID-as-rowname layout tax_table() and the common-ASV matching
    # logic below both expect.
    tax_raw <- read_csv(taxonomy_files[[db_name]], name_repair = "minimal",
                        show_col_types = FALSE, col_types = cols(.default = col_character()))
    if (ncol(tax_raw) < 2L || anyNA(names(tax_raw)) ||
        any(trimws(names(tax_raw)) == "") || anyDuplicated(names(tax_raw))) {
        stop(db_name, " taxonomy table must have a unique ASV identifier column and at least one rank column.")
    }
    expected_asv_id_names <- c("ASV_ID", "ASV ID")
    if (!names(tax_raw)[[1]] %in% expected_asv_id_names) {
        stop(db_name, " taxonomy table's first column must be named ASV_ID (or ASV ID), not '",
             names(tax_raw)[[1]], "'.")
    }
    taxonomy_asv_ids <- tax_raw[[1]]
    if (!length(taxonomy_asv_ids) || anyNA(taxonomy_asv_ids) ||
        any(trimws(taxonomy_asv_ids) == "") || anyDuplicated(taxonomy_asv_ids)) {
        stop(db_name, " taxonomy table must contain non-empty, unique ASV identifiers.")
    }
    tax_table <- as.data.frame(tax_raw[-1], stringsAsFactors = FALSE,
                               check.names = FALSE)
    rownames(tax_table) <- taxonomy_asv_ids
    ...
    # Step 5's taxonomy CSV keeps "Species" as the raw, uncorrected
    # addSpecies() output (so unresolved ASVs are genuine NA) and provides
    # the backfilled label (e.g. "Genus sp." or "unclassified Family") in a
    # separate "Corrected Species" column -- see the "Format Taxonomy
    # Tables" section of Step 5. Every species-level step below
    # (tax_glom(taxrank = "Species"), rank_names(), chart titles, the
    # column dictionary) expects a fully-populated "Species" column, so the
    # corrected values are substituted in here, once, right after loading;
    # everything downstream continues to just say "Species" and behaves
    # exactly as it did before Step 5 started keeping the two separately.
    corrected_species_column <- intersect(c("Corrected Species", "Corrected_Species"),
                                          colnames(tax_table))
    if (length(corrected_species_column) == 1L) {
        tax_table$Species <- tax_table[[corrected_species_column]]
        tax_table[[corrected_species_column]] <- NULL
    } else if (length(corrected_species_column) > 1L) {
        stop(db_name, " taxonomy table contains both Corrected Species column variants.")
    }

    taxonomy_tables[[db_name]] <- tax_table

    if (nrow(tax_table) == 0L || anyDuplicated(rownames(tax_table))) {
        stop(db_name, " taxonomy table must contain at least one unique ASV row.")
    }
    required_taxonomic_ranks <- c("Genus", "Species")
    missing_taxonomic_ranks <- setdiff(required_taxonomic_ranks, colnames(tax_table))
    if (length(missing_taxonomic_ranks) > 0L) {
        stop(db_name, " taxonomy table is missing rank(s) used later in this notebook: ",
             paste(missing_taxonomic_ranks, collapse = ", "))
    }

    cat("\n", db_name, "Taxonomy Table:\n",
        "  ASVs:", nrow(tax_table), "\n",
        "  Taxonomic ranks:", paste(colnames(tax_table), collapse = ", "), "\n")
}
```

## Load Sample Metadata (Optional) {#load-metadata}

If `metadata_path` was set above, this chunk loads it (auto-detecting a tab- or comma-separated file from its extension -- see [data/README.md](../../data/README.md) for the required format) and checks that its sample identifiers actually match the abundance table's samples -- a phyloseq object is still built without metadata, but any downstream grouping/faceting by a metadata variable requires this to succeed.
...
If `include_tree` is `TRUE` and Step 6's output is present, this chunk loads the Newick tree so it can be incorporated into the phyloseq object(s) below (enabling phylogeny-aware analyses like UniFrac downstream); it is skipped gracefully, with an explanatory message, if Step 6 was not run.
...
Every combination of taxonomy database x available abundance source gets its own phyloseq object, keyed by a `combo_id` of the form `<db_name>_<abundance_source>` (e.g. `SILVA_raw_counts`, `SILVA_copy_number_corrected`, `GTDB_microbial_load_corrected`). A parallel `phyloseq_combo_meta` list stores each combo's display metadata (taxonomy database, full abundance-source name, and its Excel-safe short label) so every downstream section can look up how to label its output without recomputing anything.
...
```{r echo=FALSE}
cat("\nTotal phyloseq objects built:", length(phyloseq_objects), "\n",
    "-", paste(names(phyloseq_objects), collapse = ", "), "\n")

if (length(phyloseq_objects) == 0L) {
    stop("No phyloseq objects could be built from the requested taxonomy and abundance inputs.")
}

# All files are written into the isolated staging directory. The prior final
# directory remains untouched until the complete new run has been verified.
```

## Explore Phyloseq Objects {#explore-phyloseq}

For every phyloseq object built above, this chunk prints basic composition statistics (taxa/sample counts, total reads) and, per taxonomic rank, how many ASVs could not be classified -- a quick sanity check before moving on to aggregation and visualization.
...
This chunk writes each phyloseq object built above to its own `.RData` file, inside that combination's database-specific `phyloseq_objects/` subfolder (e.g. `SILVA/phyloseq_objects/phyloseq_object_silva_raw_counts.RData`, `GTDB/phyloseq_objects/phyloseq_object_gtdb_raw_counts.RData`), so each one can be reloaded independently in downstream analysis scripts without re-running this notebook.

```{r save-phyloseq, results='asis'}
...
For every phyloseq object built above, this chunk collapses ASVs sharing the same Genus assignment into a single row using phyloseq's [tax_glom()](https://joey711.github.io/phyloseq/) (`NArm = FALSE`, so unclassified ASVs are kept as their own group rather than dropped), then also computes a genus-level relative-abundance version for later visualization.
...
This chunk ranks each combination's genera by total abundance in that source table and keeps the top `n_top_taxa` (set in [Configuration](#define-data-params)), previewing them as an interactive table and exporting the full ranking to a per-combination Excel sheet. **Ranking basis**: "total abundance" here means each genus's abundance **summed across every sample** within that taxonomy database x abundance source combination -- one shared, dataset-wide top `n_top_taxa`, not a separate top `n_top_taxa` chosen independently for each individual sample. A genus that is highly abundant in only one compositionally distinct sample (e.g. a control or an outlier) can therefore still fall outside this shared ranking, even though it dominates that one sample's own composition -- such a genus is simply absent from that combination's barplot and Excel sheet, rather than shown with a near-zero value. Genera with an unclassified (`NA`) Genus assignment are excluded from the ranking/selection itself -- `tax_glom(..., NArm = FALSE)` in [Aggregate at Genus Level](#aggregate-genus) deliberately collapses every unclassified ASV into one row rather than dropping it, and that row is often abundant enough to otherwise occupy a top-N slot that a named genus should have. `Relative_Abundance` is still computed against the *total* community (including unclassified reads), so the percentages shown remain a true share of the whole sample rather than being inflated by excluding the unclassified fraction from the denominator too.
...
This chunk defines a reusable helper, `create_taxonomy_barplot()`, called once per taxonomy database x abundance source combination in the section below -- centralizing the plot styling and the interactive HTML export in one place instead of repeating it at the call site.

```{r define-barplot-function}
...
So the same genus is always drawn in the same color in every barplot below -- rather than each combination's plot independently re-deriving its own color assignment from only its own subset of taxa -- this chunk builds ONE color palette shared across every call to `create_taxonomy_barplot()`, keyed by genus name. It is built from the union of every genus appearing in ANY combination's top `n_top_taxa` selection ([Identify Top Genera](#top-genera)), sorted alphabetically so the color assigned to each genus is reproducible and does not depend on which combination happened to be processed first.

```{r assign-genus-colors}
...
For every combination, this chunk generates a single interactive barplot -- relative abundance restricted to the top `n_top_taxa` genera (excluding any unclassified/NA genus -- see [Identify Top Genera](#top-genera)) -- exporting it as a self-contained HTML widget and displaying it inline. Every plot uses the same genus-to-color mapping built in [Assign Consistent Genus Colors](#genus-colors), so a given genus is always the same color no matter which combination's plot it appears in, making a direct visual, genus-by-genus comparison across plots meaningful. Absolute-abundance, full (non-restricted) relative-abundance, and any species-level barplot are intentionally not generated: absolute counts mostly reflect sequencing depth rather than biology in compositional 16S data, an unrestricted genus- or species-level barplot becomes visually unreadable once the categorical color palette runs out of distinguishable hues, and species-level assignments from short-read 16S amplicons are typically too fragmented to be worth plotting routinely (the underlying Genus- and Species-level data are still fully available -- for Genus, in the [Top Genera](#top-genera) Excel sheet and `genus_objects`/`genus_relative`; for Species, by running `tax_glom(ps, taxrank = "Species")` yourself on the saved [phyloseq object](#save-phyloseq)). Barplots are written into that combination's database-specific `barplots/` subfolder (e.g. `SILVA/barplots/`, `GTDB/barplots/`) under this notebook's output folder, so each database's set of interactive files can be browsed or shared independently.
...
# per-iteration print()/cat()) is required for every plot to actually
# render.
...
For every combination, this chunk tabulates each sample's total abundance in the units of its source table and its observed ASV richness, previews the table sorted by total abundance, and exports the full table to Excel. For raw counts the total is reads; for corrected sources it is a fractional copy-number-corrected total or a microbial-load-scaled total, rounded to 1 decimal place.
...
For every taxonomy database, this chunk counts how many distinct taxa are observed at each taxonomic rank and what fraction of ASVs were successfully classified at that rank, previews the result, and exports it to Excel as a single `Tax_Summary` sheet. The summary describes the ASVs retained in that database's raw-count phyloseq object after any taxonomy and tree matching, rather than claiming to describe ASVs that were pruned during construction.
...
Each database workbook records the exact inputs, checksums, configuration, software versions, and ASV retention for every abundance object produced in this run.

```{r run-provenance, include=FALSE}
run_provenance_by_db <- list()
for (db_name in names(taxonomy_tables)) {
    combo_ids <- names(phyloseq_combo_meta)[vapply(
        phyloseq_combo_meta, function(x) identical(x$db_name, db_name), logical(1)
    )]
    provenance_rows <- lapply(combo_ids, function(combo_id) {
        meta <- phyloseq_combo_meta[[combo_id]]
        source_def <- abundance_source_definitions[[meta$abundance_source]]
        data.frame(
            Generated_At = format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z"),
            Combination = combo_id,
            Abundance_File = as.character(source_def$path),
            Abundance_MD5 = unname(tools::md5sum(source_def$path)),
            Provenance_File = if (is.null(source_def$provenance)) NA_character_ else as.character(source_def$provenance),
            Provenance_MD5 = if (is.null(source_def$provenance)) NA_character_ else unname(tools::md5sum(source_def$provenance)),
            Taxonomy_File = as.character(taxonomy_files[[db_name]]),
            Taxonomy_MD5 = unname(tools::md5sum(taxonomy_files[[db_name]])),
            Metadata_File = if (is.null(metadata_path)) NA_character_ else as.character(metadata_path),
            Metadata_MD5 = if (is.null(metadata_path)) NA_character_ else unname(tools::md5sum(metadata_path)),
            Tree_File = if (is.null(phylo_tree)) NA_character_ else as.character(tree_path),
            Tree_MD5 = if (is.null(phylo_tree)) NA_character_ else unname(tools::md5sum(tree_path)),
            Input_ASVs = meta$input_asvs,
            Taxonomy_Matched_ASVs = meta$taxonomy_matched_asvs,
            Final_ASVs = meta$final_asvs,
            Top_Taxa = n_top_taxa,
            Phyloseq_Version = as.character(packageVersion("phyloseq")),
    ...
    run_provenance_by_db[[db_name]] <- do.call(rbind, provenance_rows)
    add_sheet_to_excel(db_excel_path(db_name), "Run_Provenance",
                       run_provenance_by_db[[db_name]], rownames = FALSE, overwrite = TRUE)
}
```

## Document and Export Column Dictionary {#column-dictionary}

Every `Top_Genera_<source_label>` and `Sample_Stats_<source_label>` sheet (one per abundance source processed) shares an identical column layout within a database's own workbook — only the underlying values differ; `Tax_Summary` is written once per database and needs no such per-source repetition. Build one `descriptions` lookup per sheet family and reuse it for every abundance source's copy of that sheet, then combine everything into a trailing `Column_Dictionary` sheet, appended separately to *each* taxonomy database's own summary workbook, so every workbook documents itself independently.

```{r build-column-dictionary, include=FALSE}
# Column descriptions for the "Top_Genera_<source_label>" sheet family.
# Columns come from `top_genera_df`, built in the "Identify Top Genera" section.
top_genera_descriptions <- c(
    Rank                = "Integer rank (1 = most abundant) of this genus among the top n_top_taxa CLASSIFIED genera by summed abundance within its taxonomy database x abundance source combination -- ASVs with an unclassified (NA) Genus assignment are excluded from this ranking (see the Identify Top Genera section). Row order in the sheet already reflects this rank.",
    ASV_ID              = "Identifier of the representative ASV that tax_glom() retained for this genus after collapsing all ASVs sharing the same Genus assignment into a single aggregated row.",
    Genus               = "Genus-level taxonomic assignment for this aggregated group, taken from the Genus column of the taxonomy table. Never NA in this sheet, since unclassified genera are excluded from the top-N ranking.",
    Total_Abundance     = "Total abundance summed across all samples for this genus, i.e. taxa_sums() on the genus-aggregated phyloseq object. Units follow the source table: reads, copy-number-corrected abundance, or microbial-load-scaled abundance. Raw counts are whole numbers of reads; copy-number-corrected and microbial-load-corrected values are rounded to 1 decimal place.",
    Relative_Abundance  = "Total_Abundance for this genus expressed as a percentage of the summed abundance of ALL genera (including any unclassified/NA genus) in the same taxonomy database x abundance source combination, rounded to 2 decimal places -- i.e. a true share of the whole community, not just of the classified fraction."
)

# Column descriptions for the "Sample_Stats_<source_label>" sheet family.
# Columns come from `sample_stats`, built in the "Per-Sample Read Counts" section.
sample_stats_descriptions <- c(
    SampleID        = "Sample name as stored in the phyloseq object (sample_names()).",
    Total_Abundance = "Total abundance summed across ASVs for this sample, i.e. sample_sums(). Units follow the source table: reads, copy-number-corrected abundance, or microbial-load-scaled abundance. Raw counts are whole numbers of reads; copy-number-corrected and microbial-load-corrected values are rounded to 1 decimal place.",
    Observed_ASVs   = "Number of ASVs with a non-zero count in this sample — the observed taxa richness for that sample.",
    Pct_Total_Abundance = "Total_Abundance for this sample expressed as a percentage of summed Total_Abundance across all samples in the same taxonomy database x abundance source combination, rounded to 2 decimal places."
)

# Column descriptions for the "Tax_Summary" sheet (one per database, not one
# per abundance source -- see the Taxonomy Summary section).
# Columns come from `tax_summary`, built in the "Taxonomy Summary" section.
tax_summary_descriptions <- c(
    Rank            = "Taxonomic rank name (e.g. Kingdom, Phylum, Class, Order, Family, Genus, Species) as reported by rank_names() on the phyloseq object.",
    Unique_Taxa     = "Number of distinct, non-NA taxonomic labels observed at this rank across all ASVs in the phyloseq object.",
    NA_Count        = "Number of ASVs that could not be classified at this rank (an NA value in the taxonomy table). NAs are expected and are retained rather than dropped, since tax_glom() is run with NArm = FALSE elsewhere in this notebook.",
    Classified_Pct  = "Percentage of ASVs successfully classified at this rank, computed as (ntaxa - NA_Count) / ntaxa * 100 and rounded to 1 decimal place."
)

# Build one dictionary entry per sheet actually written, nested first by
# taxonomy database and then by sheet name -- R auto-vivifies each
# column_dictionary_list[[db_name]] sub-list on first assignment, so no
# separate initialization per database is needed. Column structure never
# varies across abundance sources within a database, so a single
# descriptions lookup per sheet family documents every
# "<family>_<source_label>" sheet in that database's own workbook. Tax_Summary
# is the exception -- it is written once per database (not once per
# abundance source), so its loop below is keyed directly by db_name rather
# than by combo_id.
column_dictionary_list <- list()

run_provenance_descriptions <- setNames(
    c("Generation timestamp with timezone.", "Taxonomy database and abundance-source identifier.",
      "Abundance input path.", "MD5 checksum of the abundance input.",
      "Optional upstream provenance-workbook path.", "MD5 checksum of the upstream provenance workbook.",
      "Taxonomy input path.", "MD5 checksum of the taxonomy input.",
      "Optional metadata input path.", "MD5 checksum of the metadata input.",
      "Optional tree input path.", "MD5 checksum of the tree input.",
      "Number of ASVs in the abundance input.", "Number matching the taxonomy table.",
      "Number retained in the final phyloseq object.", "Configured number of top genera.",
      "phyloseq package version.", "R version."),
    names(run_provenance_by_db[[1]])
)

for (db_name in names(run_provenance_by_db)) {
    column_dictionary_list[[db_name]][["Run_Provenance"]] <- build_column_dictionary(
        sheet_name = "Run_Provenance", data = run_provenance_by_db[[db_name]],
        descriptions = run_provenance_descriptions,
        workbook_path = db_excel_path(db_name), rownames = FALSE
    )
}

for (combo_id in names(top_genera_df_list)) {
    meta <- phyloseq_combo_meta[[combo_id]]
    sheet_name <- paste0("Top_Genera_", meta$source_label)
    column_dictionary_list[[meta$db_name]][[sheet_name]] <- build_column_dictionary(
        sheet_name = sheet_name,
        data = top_genera_df_list[[combo_id]],
        descriptions = top_genera_descriptions,
        workbook_path = db_excel_path(meta$db_name),
        rownames = FALSE
    )
}

for (combo_id in names(sample_stats_list)) {
    meta <- phyloseq_combo_meta[[combo_id]]
    sheet_name <- paste0("Sample_Stats_", meta$source_label)
    column_dictionary_list[[meta$db_name]][[sheet_name]] <- build_column_dictionary(
        sheet_name = sheet_name,
        data = sample_stats_list[[combo_id]],
        descriptions = sample_stats_descriptions,
        workbook_path = db_excel_path(meta$db_name),
        rownames = FALSE
    )
}
...
for (db_name in names(tax_summary_list)) {
    sheet_name <- "Tax_Summary"
    column_dictionary_list[[db_name]][[sheet_name]] <- build_column_dictionary(
        sheet_name = sheet_name,
        data = tax_summary_list[[db_name]],
        descriptions = tax_summary_descriptions,
        workbook_path = db_excel_path(db_name),
        rownames = FALSE
    )
}

# Collapse each database's per-sheet dictionary entries into one combined
# data frame, keyed by db_name -- one combined dictionary per database.
column_dictionary_by_db <- lapply(column_dictionary_list, function(sheets) do.call(rbind, sheets))
```

```{r export-column-dictionary, include=FALSE}
# Write each database's combined dictionary as the final trailing sheet in
# that database's own workbook, after every data sheet has already been
# written to it.
for (db_name in names(column_dictionary_by_db)) {
    add_sheet_to_excel(
        db_excel_path(db_name),
        "Column_Dictionary",
        column_dictionary_by_db[[db_name]],
        rownames = FALSE,
        overwrite = TRUE
    )
    cat("Column_Dictionary sheet added to:", db_excel_path(db_name), "\n")
}
```

```{r verify-and-publish, include=FALSE}
# Verify the complete staged deliverable set before replacing the previous run.
for (combo_id in names(phyloseq_objects)) {
    meta <- phyloseq_combo_meta[[combo_id]]
    object_path <- here(db_phyloseq_folder(meta$db_name), paste0(
        "phyloseq_object_", tolower(meta$db_name), "_", meta$abundance_source, ".RData"))
    check_environment <- new.env(parent = emptyenv())
    loaded <- if (file_exists(object_path)) load(object_path, envir = check_environment) else character()
    if (!identical(loaded, "ps") || !inherits(check_environment$ps, "phyloseq")) {
        stop("Final staged object is missing or unreadable: ", object_path)
    }
    if (combo_id %in% names(top_genera_list)) {
        plot_path <- here(db_barplot_folder(meta$db_name), paste0(
            "barplot_genus_top", n_top_taxa, "_", meta$abundance_source, ".html"))
        if (!file_exists(plot_path) || file_info(plot_path)$size <= 0) {
            stop("Final staged barplot is missing or empty: ", plot_path)
        }
    }
}
...
for (db_name in names(taxonomy_tables)) {
    workbook_path <- db_excel_path(db_name)
    required_sheets <- c("Tax_Summary", "Run_Provenance", "Column_Dictionary")
    if (!file_exists(workbook_path) ||
        !all(required_sheets %in% openxlsx::getSheetNames(workbook_path))) {
        stop("Final staged workbook is missing, unreadable, or incomplete: ", workbook_path)
    }
}

backup_output_folder <- here(results_folder, ".9_phyloseq_object_previous")
if (fs::dir_exists(backup_output_folder)) fs::dir_delete(backup_output_folder)
if (fs::dir_exists(final_output_folder)) {
    fs::file_move(final_output_folder, backup_output_folder)
}
publish_succeeded <- FALSE
tryCatch({
    fs::file_move(output_folder, final_output_folder)
    publish_succeeded <- TRUE
}, error = function(e) {
    if (!fs::dir_exists(final_output_folder) && fs::dir_exists(backup_output_folder)) {
        fs::file_move(backup_output_folder, final_output_folder)
    }
    stop("Could not publish the verified Step 9 run; the prior output was restored. ", conditionMessage(e))
})
if (publish_succeeded && fs::dir_exists(backup_output_folder)) fs::dir_delete(backup_output_folder)
if (publish_succeeded && fs::file_exists(step9_checkpoint_path)) fs::file_delete(step9_checkpoint_path)
output_folder <- final_output_folder
cat("Verified and published the complete Step 9 deliverable set.\n")
```

```{r column-dictionary-links, echo=FALSE, results='asis'}
# cat()-ed per iteration (rather than collected and cat()-ed once after the
# loop) because a bare for loop always returns invisible(NULL) regardless of
# its body -- see the IMPORTANT note in render_output_links_function.R. Note
# that must be cat(), not print(): render_output_links() returns a
# knitr::asis_output() object, and print() on that (rather than cat()) falls
# back to base R's print.default(), which shows the object's raw character
# content and class/attribute metadata as literal text instead of rendering
# the Markdown link -- see the comment in the save-phyloseq chunk above for
# the full explanation.
for (db_name in names(column_dictionary_by_db)) {
    cat(render_output_links(
        db_excel_path(db_name),
        labels = paste(db_name, "summary workbook (Excel)")
    ))
}
```
...
The tree below lists every file this notebook has written to its own output folder, [results/9_phyloseq_object/](../../results/9_phyloseq_object/), as a clickable, portable link (relative to this notebook's own location) -- built live from what is actually on disk at knit time via the project's shared [render_output_tree_function.R](../functions/render_output_tree_function.R) helper. The exact set of `.RData` phyloseq objects, interactive `.html` barplots, and per-database Excel workbooks depends on which taxonomy database(s) and optional upstream steps (6-8) were used in this run, so most filenames here are dynamically generated rather than fixed -- only the database subfolders (top-level, `phyloseq_objects/`, and `barplots/`) and each database's summary workbook get an inline description; every other file's name is already self-descriptive (e.g. `phyloseq_object_silva_raw_counts.RData`, `barplot_genus_top10_raw_counts.html`).

```{r output-summary, echo=FALSE}
render_output_tree(
  output_folder,
  descriptions = c(
    "SILVA" = "SILVA-based phyloseq objects, barplots, and summary workbook",
    "GTDB" = "GTDB-based phyloseq objects, barplots, and summary workbook",
    "SILVA/phyloseq_objects" = "SILVA phyloseq objects (.RData)",
    "SILVA/barplots" = "SILVA interactive barplots (HTML)",
    "GTDB/phyloseq_objects" = "GTDB phyloseq objects (.RData)",
    "GTDB/barplots" = "GTDB interactive barplots (HTML)",
    "SILVA/phyloseq_summary_silva.xlsx" = "SILVA phyloseq object assembly summary (Excel workbook)",
    "GTDB/phyloseq_summary_gtdb.xlsx" = "GTDB phyloseq object assembly summary (Excel workbook)"
  )
)
```
...
::: {.alert .alert-info}
**Optional**: For interactive exploration of a phyloseq object, you can use the Shiny-Phyloseq web application. Uncomment and run the code below to launch it.
:::

```{r shiny-phyloseq, eval=FALSE}
...
This is the last notebook of the DADA2 16S pipeline itself, so there is no further DADA2 notebook to run -- the phyloseq object(s) exported above are analysis-ready for whatever community-ecology workflow you use next (alpha/beta diversity, ordination, differential abundance testing, etc.). If your next question is about the community's *functional* potential rather than its taxonomic composition, this pipeline's natural companion project is the separate **PICRUSt2_16S_Functional_Inference_Workflow** repository: it takes this pipeline's ASV table and representative sequences as input and uses [PICRUSt2](https://github.com/picrust/picrust2/wiki) to predict each sample's functional gene content (e.g. KEGG pathways, EC numbers) via phylogenetic placement and hidden-state prediction. It is a separate, standalone repository rather than part of this one, so it is named here rather than linked.
...
-   [phyloseq: Explore microbiome profiles using R](https://joey711.github.io/phyloseq/index.html) — official documentation for the package this entire notebook is built on.
-   [phyloseq tutorials](https://joey711.github.io/phyloseq/tutorials-index.html)
-   [Shiny-phyloseq: An interactive web application](https://joey711.github.io/shiny-phyloseq/) — the optional interactive explorer launched in [Interactive Exploration with Shiny-Phyloseq](#interactive-exploration-with-shiny-phyloseq) above.

## Databases

-   Quast C, Pruesse E, Yilmaz P, et al. (2013). The SILVA ribosomal RNA gene database project: improved data processing and web-based tools. *Nucleic Acids Research*, 41(D1):D590-D596. <https://doi.org/10.1093/nar/gks1219> — [SILVA](https://www.arb-silva.de/) is one of the two taxonomy databases this notebook can build a phyloseq object from.
-   Parks DH, Chuvochina M, Rinke C, et al. (2022). GTDB: an ongoing census of bacterial and archaeal diversity through a phylogenetically consistent, rank normalized and complete genome-based taxonomy. *Nucleic Acids Research*, 50(D1):D199-D207. <https://doi.org/10.1093/nar/gkab776> — [GTDB](https://gtdb.ecogenomic.org/) is this notebook's other supported taxonomy database.

## Related

-   [Step 5 — DADA2 Pipeline](5_dada2_pipeline.md) — this notebook's required input (ASV count table and taxonomy assignments).
-   [Step 6 — Phylogenetic Tree](6_phylogenetic_tree.md) — optional input, incorporated into the phyloseq object(s) if present.
-   [Step 7 — 16S rRNA Gene Copy Number Correction](7_copy_number_correction.md) — optional input, adds a copy-number-corrected phyloseq object per taxonomy database.
-   [Step 8 — Microbial Load Correction](8_microbial_load_correction.md) — optional input, adds a microbial-load-corrected phyloseq object per taxonomy database.
...
**Symptom**: The [Build Phyloseq Objects](#build-phyloseq) section reports fewer combinations than `number of taxonomy databases x 3`.

**Cause**: `copy_number_corrected` and/or `microbial_load_corrected` abundance tables were not found, because [Step 7](7_copy_number_correction.md) and/or [Step 8](8_microbial_load_correction.md) have not been run. This is expected -- both are optional -- and is reported as a skip message, not an error, in [Load Abundance Tables](#load-abundance).

**Actions**:

-   Run [Step 7](7_copy_number_correction.md) and/or [Step 8](8_microbial_load_correction.md) first if you want the additional phyloseq object(s), then re-run this notebook.
...
-   ASV count table uses sample names as row names instead of column names.
...
-   Process one taxonomy database at a time: temporarily rename or move the other database's taxonomy CSV ([silva_taxonomy_table.csv](../../results/5_dada2_pipeline/silva_taxonomy_table.csv) or [gtdb_taxonomy_table.csv](../../results/5_dada2_pipeline/gtdb_taxonomy_table.csv), in [results/5_dada2_pipeline/](../../results/5_dada2_pipeline/)) out of that folder before running this notebook, so [Load Taxonomy Table(s)](#load-taxonomy) only detects the one you want -- restore the file afterward to include it on a later run.
...
**Cause**: This is common and expected. Many ASVs cannot be classified to all taxonomic levels.

**Actions**:

-   Use `NArm = FALSE` in `tax_glom()` to retain unclassified taxa.
-   Consider using less stringent classification thresholds in [Step 5](5_dada2_pipeline.md).
