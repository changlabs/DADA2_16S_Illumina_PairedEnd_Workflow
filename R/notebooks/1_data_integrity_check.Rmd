---
title: "Step 1: FASTQ Data Integrity Check"
output:
  github_document:
    toc: true
    toc_depth: 3
...
This notebook is **Step 1** of the 16S rRNA sequencing workflow. It performs an optional but recommended structural integrity check of paired-end [FASTQ](https://doi.org/10.1093/nar/gkp1137) files before downstream processing. It confirms that files are readable and parseable, matches forward and reverse files by name, counts complete FASTQ records, and compares paired read counts. It does not replace checksum verification against values supplied by a sequencing facility or data repository.

## Prerequisites

Before running this notebook, ensure that your raw FASTQ files (`.fastq`, `.fq`, and their gzip-compressed variants) are present in the [data/fastq/](../../data/fastq/) input directory.

## What This Notebook Does

1.  **File Discovery**: Scans the input folder for `.fastq`, `.fq`, and gzip-compressed variants
2.  **Pair Matching**: Identifies and matches forward and reverse reads using the configured filename tokens.
...
4.  **Concordance Check**: Verifies that paired files contain the same number of reads. Equal counts support correct pairing but do not prove sample identity.
5.  **Report Generation**: Exports a summary table to an Excel file for documentation and review
...
-   **Naming Convention**: The default configuration expects:
    -   Forward reads: `{sample_id}_L001_R1_001` followed by `.fastq`, `.fq`, or a gzip-compressed variant
    -   Reverse reads: `{sample_id}_L001_R2_001` followed by `.fastq`, `.fq`, or a gzip-compressed variant
    -   Edit `forward_token` and `reverse_token` in the configuration section when your files use another consistent convention.
-   **Format**: Standard FASTQ format (4-line records per read)

## Expected Output

-   An Excel file ([data_integrity_check.xlsx](../../results/1_data_integrity_check/data_integrity_check.xlsx)) saved to [results/1_data_integrity_check/](../../results/1_data_integrity_check/)
...
The chunk below loads every R package this notebook depends on, plus four helper functions shared across the pipeline: Excel writing, column-dictionary documentation, clickable output links, and the output-tree summary (each sourced from [R/functions/](../functions/)).
...
Configure the regular expression and tokens used to identify and pair FASTQ files.
...
This function extracts the display identifier used consistently by this workflow. By convention, it uses the first underscore-delimited filename component; change this configuration if underscores are part of your biological sample IDs or change the SampleIDs.
...
This function creates a matching key to pair forward and reverse reads. It replaces the read direction token with a generic placeholder, allowing forward and reverse files to share the same key.

```{r func-derive-pair-key}
...
This function parses an entire FASTQ file with [ShortRead](https://doi.org/10.1093/bioinformatics/btp450). A successful parse supplies the record count and marks the file as valid; malformed records, truncated gzip streams, empty files, missing files, and other read errors are reported as invalid.

```{r func-inspect-fastq}
...
Render interactive tables in HTML and ordinary Markdown tables for non-HTML output formats.
...
Scan the input directory for all FASTQ files matching our given extension patterns for forward and reverse reads.
...
Build a data.table containing file paths and metadata for all discovered FASTQ files.
...
Classify each file as forward (Read 1) or reverse (Read 2) based on the filename tokens.

```{r classify-read-direction}
...
Separate files into forward and reverse sets, and check for ambiguous classifications.

```{r separate-read-sets}
...
Create standardized keys that allow matching of forward and reverse files.

```{r generate-pair-keys}
...
For each file, parse the complete FASTQ once, collect its record count, calculate its size, and record whether parsing succeeded.

::: {.alert .alert-info}
**Note**: This step may take several minutes for large datasets because every FASTQ record is parsed.
:::

```{r echo=FALSE}
cat("Parsing FASTQ files and counting records...\n",
    "This may take a few minutes for large files.\n\n")
```

```{r compute-quality-metrics, message=FALSE}
...
Check that paired files have matching read counts—a critical quality control step.

```{r verify-read-concordance}
...
cat(
  ...
  "\n- Read Counts:\n",
  "  Forward files - Total reads:", format(sum(paired_dt$reads_fwd, na.rm = TRUE), big.mark = ","), "\n",
  "  Reverse files - Total reads:", format(sum(paired_dt$reads_rev, na.rm = TRUE), big.mark = ","), "\n",
  "\n- File Sizes:\n",
  "  Forward files - Total size:", round(sum(paired_dt$size_mb_fwd, na.rm = TRUE) / 1000, 2), "GB\n",
  "  Reverse files - Total size:", round(sum(paired_dt$size_mb_rev, na.rm = TRUE) / 1000, 2), "GB\n",
  "\n- Validation Status:\n",
  "  Valid forward files:", sum(paired_dt$valid_fwd, na.rm = TRUE), "/", sum(!is.na(paired_dt$absolute_path_fwd)), "\n",
  "  Valid reverse files:", sum(paired_dt$valid_rev, na.rm = TRUE), "/", sum(!is.na(paired_dt$absolute_path_rev)), "\n",
  "\n- Paired Read Concordance:\n",
  "  Matching read counts:", sum(paired_dt$reads_match, na.rm = TRUE), "/", sum(!is.na(paired_dt$reads_match)), "\n",
  "  Mismatched pairs:", sum(paired_dt$reads_match == FALSE, na.rm = TRUE), "\n"
)

overall_integrity_status <- if (
  nrow(ambiguous_dt) == 0L &&
  length(duplicate_sample_ids) == 0L &&
  nrow(paired_dt) > 0L &&
  all(paired_dt$integrity_status == "PASS")
) "PASS" else "FAIL"

cat("\n- Overall integrity status:", overall_integrity_status, "\n")
```
...
Export the integrity check results to an Excel file for documentation and sharing, using the project's shared Excel-writing helper, [add_sheet_to_excel_function.R](../functions/add_sheet_to_excel_function.R).
...
Build a trailing `Column_Dictionary` sheet that documents every column of every sheet already written to `output_excel_path`, using the project's shared [build_column_dictionary_function.R](../functions/build_column_dictionary_function.R) helper, then append it to the workbook as the final sheet.

```{r build-column-dictionary, include=FALSE}
# Named lookup vector mapping each column of export_dt (as written to the
# output_excel_sheet, i.e. "Data_Integrity_Check", sheet) to a human-readable
# explanation of what that column means and how it was derived.
data_integrity_check_column_descriptions <- c(
  `SampleID`           = "Display identifier extracted from the first underscore-delimited FASTQ filename component, following the naming convention shared by this workflow. Forward/reverse matching uses the full filename-derived pair_key rather than this shortened display value.",
  `Pair Key`           = "Full filename-derived pairing key after removing the FASTQ extension and replacing the terminal read-direction token; retained to disambiguate shortened SampleID values.",
  `Forward File`       = "Filename of the forward (R1) FASTQ mate; NA when missing.",
  `Reads (Forward)`    = "Total number of sequencing reads counted in the forward (R1) FASTQ file via ShortRead::countFastq(); NA if the forward file was missing or could not be read.",
  `Size (MB, Forward)` = "Size of the forward (R1) FASTQ file on disk, in megabytes, rounded to 2 decimal places; a rough sanity check that file size is consistent with the reported read count.",
  `Forward Issue`      = "FASTQ parsing or file-access problem for the forward mate; NA when valid, or an explicit missing-mate message.",
  `Reverse File`       = "Filename of the reverse (R2) FASTQ mate; NA when missing.",
  `Reads (Reverse)`    = "Total number of sequencing reads counted in the reverse (R2) FASTQ file via ShortRead::countFastq(); NA if the reverse file was missing or could not be read.",
  `Size (MB, Reverse)` = "Size of the reverse (R2) FASTQ file on disk, in megabytes, rounded to 2 decimal places; a rough sanity check that file size is consistent with the reported read count.",
  `Reverse Issue`      = "FASTQ parsing or file-access problem for the reverse mate; NA when valid, or an explicit missing-mate message.",
  `Valid (Forward)`    = "TRUE if the non-empty forward (R1) FASTQ file was parsed completely by ShortRead::countFastq(); FALSE if parsing failed; NA if the forward mate was missing.",
  `Valid (Reverse)`    = "TRUE if the non-empty reverse (R2) FASTQ file was parsed completely by ShortRead::countFastq(); FALSE if parsing failed; NA if the reverse mate was missing.",
  `Reads Match`        = "TRUE if the parsed forward and reverse FASTQ files contain equal record counts. Equal counts support pair completeness but do not prove sample identity; NA if either count is unavailable.",
  `Integrity Status`   = "PASS only when both mates parse and their record counts match; otherwise an explicit row-level failure reason.",
  `Overall Status`     = "PASS only when every pair passes and there are no ambiguous files or duplicate shortened SampleID values; otherwise FAIL."
)

file_inventory_column_descriptions <- c(
  `Filename`       = "Filename of every FASTQ-like input discovered in data/fastq, including ambiguous or unclassified files.",
  `Absolute Path`  = "Absolute filesystem path used to inspect the discovered file.",
  `Classification` = "Forward, Reverse, ambiguous because both tokens matched, or unclassified because neither token matched."
)

# Build the Sheet / Column / Explanation rows for both data sheets already
# written to output_excel_path, then row-bind them into one dictionary.
column_dictionary_sheets <- list(
  build_column_dictionary(
    sheet_name    = output_excel_sheet,
    data          = export_dt,
    descriptions  = data_integrity_check_column_descriptions,
    workbook_path = output_excel_path,
    rownames      = FALSE
  ),
  build_column_dictionary(
    sheet_name    = "File_Inventory",
    data          = file_inventory_dt,
    descriptions  = file_inventory_column_descriptions,
    workbook_path = output_excel_path,
    rownames      = FALSE
  )
)
...
# Append the Column_Dictionary sheet last, after every data sheet has already
# been written to this workbook.
add_sheet_to_excel(
  workbook_path = output_excel_path,
...
render_output_links(output_excel_path, labels = "FASTQ data integrity report (Excel workbook)")
```
...
The tree below lists every file this notebook has written to its own output folder, [`results/1_data_integrity_check/`](../../results/1_data_integrity_check/), as a clickable, portable link (relative to this notebook's own location) with a short description -- built live from what is actually on disk at knit time via the project's shared [`render_output_tree_function.R`](../functions/render_output_tree_function.R) helper, so it always matches this run's real output rather than a hand-maintained list.

```{r output-summary, echo=FALSE}
render_output_tree(
  output_folder,
  descriptions = c(
    "data_integrity_check.xlsx" = "FASTQ inventory, pairing, read-count, validity, and overall integrity report (Data_Integrity_Check + File_Inventory + Column_Dictionary sheets)"
  )
)
```
...
This notebook's integrity check is optional but recommended before continuing. Proceed to [Step 2 — FastQC Quality Reports](2_fastqc_quality_reports.md), which generates FastQC and MultiQC quality reports for the same FASTQ files in [data/fastq/](../../data/fastq/); Step 2 re-derives its own file discovery and sample identification directly from that folder, so it can be run whether or not this notebook was completed first.
...
-   Cock PJA, Fields CJ, Goto N, Heuer ML, Rice PM (2010). The Sanger FASTQ file format for sequences with quality scores, and the Solexa/Illumina FASTQ variants. *Nucleic Acids Research*, 38(6):1767-1771. <https://doi.org/10.1093/nar/gkp1137>
-   Morgan M, Anders S, Lawrence M, Aboyoun P, Pagès H, Gentleman R (2009). ShortRead: a bioconductor package for input, quality assessment and exploration of high-throughput sequence data. *Bioinformatics*, 25(19):2607-2608. (provides `countFastq()`, used for complete-file parsing and record counts in this notebook) <https://doi.org/10.1093/bioinformatics/btp450>
...
-   Illumina, Inc. FASTQ Files Explained — background on the Illumina FASTQ naming convention (e.g. `_L001_R1_001`) this notebook's file-pairing logic is built around. <https://support.illumina.com/bulletins/2016/04/fastq-files-explained.html>

## Related

-   [Step 2 — FastQC Quality Reports](2_fastqc_quality_reports.md) — this notebook's recommended next step.
...
-   Check that files have `.fastq`, `.fastq.gz`, `.fq`, or `.fq.gz` extensions
-   Ensure files are in the directory root (not in subdirectories, as `recurse = FALSE`)
...
-   Verify file integrity using [MD5 checksums](https://www.rfc-editor.org/rfc/rfc1321)
